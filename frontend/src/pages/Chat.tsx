...
const Chat = () => {
  ...
  const handleSubmit = async ()=>{
    ...
    //send api req to backedn with new message
    ...
    <Box sx={{display:'flex',flex:1,width:'100% ', height:'100%',mt:3,gap:3}}>
      <Box sx={{display:{md:'flex',xs:'none',sm:'none'},flex:0.2,flexDirection:"column"}}>
        <Box sx={{display:'flex',width:'100%',height:"60vh", bgcolor:"rgb(17,29,39)",borderRadius:5,flexDirection:"column",mx:3}}>
          ...
          <Typography sx={{mx:'auto',fontFamily:"work sans",my:4,padding:3}}>You can ask some questions related to Knowledge, Business, Advices,
            Education, etc. But avoid sharing personal information</Typography>
            <Button onClick={handleDelete} sx={{width:"200px",my:"auto",color:"white",fontWeight:700,borderRadius:3,mx:'auto',bgcolor:red[300],
              ":hover":{
                bgcolor: red.A400
      ...
      <Box sx={{display:'flex',flex:{md:0.8,xs:1,sm:1},flexDirection:"column",padding:3}}>
        ...
        <div style={{width:"100%",borderRadius:8,backgroundColor:"rgb(17,27,29)",display:'flex',
          marginRight:'auto'
        }}>{" "}
        <input ref={inputRef}
        type='text' style={{width:"100%" , backgroundColor:"transparent",padding:"30px" , border:"none",outline:"none" ,color:"white" , fontSize:"20px"}}></input>
        <IconButton onClick={handleSubmit} sx={{al:"auto", color:"white",mx:'1'}}><IoMdSend></IoMdSend></IconButton>
      </div>
      </Box>
    </Box>
  )
