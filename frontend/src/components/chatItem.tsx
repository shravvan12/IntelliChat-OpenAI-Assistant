...
const ChatItem = ({
    ...
    return role ==='assistant' ? (<Box sx={{display:'flex',p:2,bgcolor:"#004d5612",my:2 , gap:2}}>
        ...
        <Box>
            ...
            messageBlocks.map((block)=> (isCodeBlock(block)? (<SyntaxHighlighter style={coldarkCold} language='javascript'>{block}</SyntaxHighlighter>):(<Typography fontSize={"20px"}>{block}</Typography>)))}
        </Box>
    </Box>
